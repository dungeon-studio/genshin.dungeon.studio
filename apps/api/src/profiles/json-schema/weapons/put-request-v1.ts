// SPDX-FileCopyrightText: 2026 Alex Brandt <alunduil@gmail.com>
// SPDX-License-Identifier: MIT

import type { JsonSchemaProfile } from '@/profiles/json-schema/json-schema-profile.js';

export const weaponPutRequestV1 = {
  path: '/profiles/json-schema/weapons/put-request-v1.json',
  schema: {
    $schema: 'https://json-schema.org/draft/2020-12/schema',
    title: 'Save Weapon Instance Request',
    description:
      "Request body for creating or replacing a weapon instance at a caller-chosen identifier in the user's collection",
    type: 'object',
    properties: {
      weaponId: {
        type: 'string',
        minLength: 1,
        description: 'Identifier of the weapon from game data',
      },
      refinementLevel: {
        type: 'integer',
        minimum: 1,
        maximum: 5,
        description: 'Refinement rank of the weapon instance',
      },
    },
    required: ['weaponId', 'refinementLevel'],
    additionalProperties: false,
  },
} as const satisfies JsonSchemaProfile;
