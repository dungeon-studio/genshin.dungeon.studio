...
- [.github/workflows/ci.yml](.github/workflows/ci.yml) runs everything that gates a pull request: the per-package test matrix, every pre-commit hook, and the security scans. Each is its own job, so a failure names the check. A hook added to `.pre-commit-config.yaml` is enforced there without any workflow edit.
